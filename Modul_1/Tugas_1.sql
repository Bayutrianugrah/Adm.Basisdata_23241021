...
-- Corrected SELECT statements for single column queries
SELECT p.nama as nama_customer FROM Pelanggan p;
SELECT k.nama_kategori as jenis_kategori FROM Kategori k;
SELECT t.nama_toko as nama_merchant FROM Toko t;
SELECT pr.nama as nama_barang FROM Produk pr;
SELECT tr.status as status_pemesanan FROM Transaksi tr;
SELECT dt.subtotal as total_pembelian FROM Detail_Transaksi dt;
SELECT u.rating as nilai_produk FROM Ulasan u;
SELECT p.email as email_pembeli FROM Pelanggan p;
SELECT pr.harga as harga_barang FROM Produk pr;
SELECT tr.tanggal_transaksi as waktu_pembelian FROM Transaksi tr;

-- Corrected SELECT statements for two column queries
SELECT p.id_pelanggan as id_customer, p.nama as nama_pembeli FROM Pelanggan p;
SELECT k.id_kategori as id_kat, k.nama_kategori as nama_kat FROM Kategori k;
SELECT t.id_toko as id_store, t.nama_toko as nama_store FROM Toko t;
SELECT pr.id_produk as id_item, pr.nama as nama_item FROM Produk pr;
SELECT tr.id_transaksi as id_order, tr.total_harga as jumlah_tagihan FROM Transaksi tr;
SELECT dt.id_detail as id_rincian, dt.jumlah as qty_barang FROM Detail_Transaksi dt;
SELECT u.id_ulasan as id_review, u.komentar as komentar_pembeli FROM Ulasan u;
SELECT p.alamat as lokasi_pengiriman, p.no_telpon as kontak_penerima FROM Pelanggan p;
SELECT pr.stok as sisa_stok, pr.deskripsi as deskripsi_produk FROM Produk pr;
SELECT tr.id_pelanggan as pembeli, tr.status as status_pengiriman FROM Transaksi tr;